...
exports.bookinstance_detail = asyncHandler(async (req, res, next) => {
  ...
  if (bookInstance === null) {
    // No results.
    const err = new Error("書籍のコピーは見つかりません");
    err.status = 404;
    return next(err);
  }
...
exports.bookinstance_create_post = [
  ...
  body("book", "書籍は必ず選択（指定）してください。").trim().isLength({ min: 1 }).escape(),
  body("imprint", "出版社は必ず選択（指定）してください。")
    .trim()
    .isLength({ min: 1 })
    .escape(),
  body("status").escape(),
...
exports.bookinstance_update_get = asyncHandler(async (req, res, next) => {
  ...
  if (bookInstance === null) {
    // No results.
    const err = new Error("書籍コピーが見つかりません");
    err.status = 404;
    return next(err);
  }
